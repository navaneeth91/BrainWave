...
const CourseCard = ({course}) => {
  ...
  return (
    <Link to={'/course/'+course._id} onClick={()=>scrollTo(0,0)}
     className='border border-grey-500/30 pb-6 overflow-hidden rounded-lg transition duration-300 transform hover:scale-105 hover:shadow-lg
'>
      <img src={course.courseThumbnail} alt={course.title} className='w-full' />
      <div className='p-3 text-left'>
        <h3 className='text-base font-semibold'>{course.courseTitle}</h3>
          <p className='text-lg text-gray-500'>Navaneeth</p>
          <div className='flex items-center space-x-2 '>
            ...
            <p className='text-grey-800'>{course.courseRatings.length}</p>
          </div>
